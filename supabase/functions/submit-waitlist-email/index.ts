 import { serve } from "https://deno.land/std@0.168.0/http/server.ts"
  ...
  serve(async (req) => {
    ...
    try {
      ...
      if (!supabaseUrl || !supabaseServiceKey) {
        console.error('Missing environment variables: SUPABASE_URL or 
  SUPABASE_SERVICE_ROLE_KEY')
      ...
      if (!resendApiKey) {
        ...
        console.warn('Will continue with waitlist signup but email notification will not be 
  sent')
      }
      ...
      const { email, source = 'waitlist', marketingConsent = true, metadata = {} } =
  requestData as any
      ...
      // Send confirmation email if we have the API key and it's not a duplicate 
  subscription
