...
export const PreviewContent: React.FC<PreviewContentProps> = ({
  ...
  return (
    <div className={cn(
      ...
      isMobile ? "mobile-flatten" : "scroll-container-optimized"
    )}>
      {/* SearchBar container - simplified on mobile */}
      <div className={cn(
        "w-full",
        isMobile ? "py-2 px-2" : "px-3 sm:px-6 md:px-8 lg:px-10 py-3 sm:py-4 md:py-6"
      )}>
        <motion.div
      ...
      {/* CreatorsList container - simplified on mobile */}
      <div className={cn(
        "w-full",
        isMobile 
          ? "py-3 px-2 bg-white" 
          : "px-3 sm:px-6 md:px-8 lg:px-10 bg-gradient-to-b from-transparent via-purple-50/20 to-purple-50/40 pb-6 sm:pb-8 md:pb-10 pt-4 sm:pt-6 md:pt-8"
      )}>
        <AnimatePresence>
          {isVisible && (
            <motion.div
              ...
              transition={isMobile ? undefined : { duration: 0.7, delay: 0.7 }}
            >
              <CreatorsList 
