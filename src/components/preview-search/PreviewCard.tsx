...
export const PreviewCard: React.FC<PreviewCardProps> = ({ isVisible, children }) => {
  ...
  return (
    <motion.div 
      ...
      className={cn(
        "relative rounded-lg sm:rounded-xl overflow-hidden shadow-lg sm:shadow-xl border border-zinc-200/70",
        isMobile ? "bg-white" : "bg-white/95 will-change-transform backdrop-blur-sm"
      )}
    >
      ...
      {/* Only render wavy background on desktop */}
      {!isMobile && (
        <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden rounded-lg sm:rounded-xl">
          <WavyBackground 
      ...
      {/* Content with simplified background on mobile */}
      {isMobile ? (
